...
class StockLocationOrderpoint(models.Model):
    ...
    def _get_qties_to_replenish(self, moves_by_location):
        # We need to instruct the computation of _compute_quantities_dict
        # to also compute the recommended_qty
        self_with = self
        if "average_daily_sale" in self.mapped("replenish_method"):
            self_with = self.with_context(compute_recommended_qty=True)
        return super(StockLocationOrderpoint, self_with)._get_qties_to_replenish(
            moves_by_location
        )
    ...
    def _compute_quantities_dict(self, locations, products):
        ...
        if self.env.context.get("compute_recommended_qty"):
